            ...
            {SECTIONS.map((s, i) => {
              ...
              return (
                <div key={s.label} className="flex items-center gap-1">
                  <div className={cn(
                    "flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold transition-all duration-300 whitespace-nowrap",
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100 gap-3">
              ...
              <motion.button onClick={handleNext} disabled={!canGoNext || !!isSubmitting}
                ...
                {isLast && isSubmitting
                  ...
                  : <>{isLast ? "Continue" : "Next"}<ChevronRight className="w-4 h-4" /></>
                }
              </motion.button>
            </div>
